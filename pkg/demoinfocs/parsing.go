...
import (
	...
	"github.com/bugkingzht/cs-demobox/pkg/demoinfocs/sendtables/sendtablescs2"
...
// SkipFrame advances the demo stream by one frame without parsing its contents.
// It only reads the frame header and skips the payload, so it is much faster than ParseNextFrame()
// when you need to seek forward (e.g. fast-forward to a target frame).
//
// The parser's CurrentFrame() and CurrentTime() are updated so Progress() stays consistent.
// Game state (players, entities, etc.) is not updated for the skipped frame.
//
// Returns false when the demo command is DEM_Stop (end of demo) or on unexpected EOF.
// On EOF the parser error is set to ErrUnexpectedEndOfDemo. Does not use the message queue
// or dispatch any events for the skipped frame.
//
// Must only be used when the header has already been parsed (e.g. after ParseNextFrame() or
// ParseHeader() has been called at least once).
func (p *parser) SkipFrame() (ok bool) {
	defer func() {
		if r := recover(); r != nil {
			if r == io.ErrUnexpectedEOF || r == io.EOF {
				p.setError(errors.Wrap(ErrUnexpectedEndOfDemo, "unexpected EOF"))
			} else {
				panic(r)
			}
			ok = false
	...
	// Advance the bit stream without parsing the payload (no unmarshal, no handleDemoPacket)
	p.bitReader.ReadBytes(int(size))

	// Keep CurrentFrame() and CurrentTime() / Progress() consistent
	p.gameState.handleIngameTickNumber(ingameTickNumber(int32(tick)))
	p.currentFrame++
